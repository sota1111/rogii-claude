...
SRC_MODULE = REPO_ROOT / "src" / "dtw_align.py"

_SHARED_RE = re.compile(
    r"# === BEGIN DTW_SHARED_CODE.*?# === END DTW_SHARED_CODE ===",
    re.DOTALL,
)
...
def _shared_block(path: Path) -> str:
    ...
    assert match is not None, f"DTW_SHARED_CODE markers missing in {path}"
...
class DtwSharedCodeTest(unittest.TestCase):
    """The constrained+stochastic DTW must stay byte-identical between src and kernel."""

    def test_shared_block_is_byte_identical(self) -> None:
        self.assertEqual(
            _shared_block(SRC_MODULE),
            _shared_block(KERNEL),
            "src.dtw_align and kernel DTW_SHARED_CODE diverged",
        )

    def test_kernel_defines_dtw_before_gold_pool(self) -> None:
        text = KERNEL.read_text(encoding="utf-8")
        self.assertLess(
            text.index("def predict_dtw_well"),
            text.index("def _gold_candidate_pool"),
            "predict_dtw_well must be defined before the gold candidate pool",
        )


@unittest.skipUnless(np is not None, "numpy absent")
class DtwSyntheticTest(unittest.TestCase):
    """DTW alignment on a synthetic well whose GR follows the typewell exactly."""

    @staticmethod
    def _synthetic(n: int = 360, heel: int = 240):
    ...
    def test_preserves_known_heel_rows(self) -> None:
        ...
        out = np.asarray(predict_dtw_well(rows, typewell))
    ...
    def test_is_deterministic(self) -> None:
        ...
        a = np.asarray(predict_dtw_well(rows, typewell))
        b = np.asarray(predict_dtw_well(rows, typewell))
    ...
    def test_degrades_to_finite_without_typewell(self) -> None:
        ...
        out = np.asarray(predict_dtw_well(rows, []))  # no typewell rows
        self.assertEqual(len(out), len(rows))
...
class DtwDataTest(unittest.TestCase):
    def test_matches_kernel_implementation(self) -> None:
        """kernel <-> src predict_dtw_well must be byte-identical (exec gate)."""
        from src.dtw_align import predict_dtw_well

        spec = importlib.util.spec_from_file_location("_rogii_kernel_dtw", KERNEL)
        ...
        src_out = np.asarray(predict_dtw_well(masked, typewell))
        kernel_out = np.asarray(kernel.predict_dtw_well(masked, typewell))
        self.assertEqual(src_out.shape, kernel_out.shape)
        self.assertTrue(
            np.array_equal(np.nan_to_num(src_out), np.nan_to_num(kernel_out)),
            "src.dtw_align and kernel predict_dtw_well diverged",
        )

    def test_dtw_in_gold_pool(self) -> None:
        ...
        self.assertIn("dtw", pool)
        self.assertEqual(len(pool["dtw"]), len(rows))
        # Known heel rows are preserved verbatim by the dtw candidate.
        for i, row in enumerate(masked):
            if row["TVT_input"]:
                self.assertAlmostEqual(
                    float(pool["dtw"][i]), float(row["TVT_input"]), places=6
                )
